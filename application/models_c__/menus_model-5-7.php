		...
		if($intNumItems>0)
		{
			...
			for($i=1; $i<=$intNumItems;$i++)
			{
				if($this->input->post("txtItemName".$i)!="" && $this->input->post("lstItemPage".$i)!="" && $this->input->post("rdoItemPublished".$i)!="" && $this->input->post("menuTarget".$i)!=""){
					...
					$this->db->query("INSERT INTO menuitems(menu_id, item_name, item_published, item_status, item_link, item_target, display_order) VALUES(".$intMenuId.",".$this->db->escape($strItemName).",  ".$this->db->escape($strPublished).", 'Active' , ".$this->db->escape($strCustomLink)." , ".$this->db->escape($menuTarget)." ,'$k')");
