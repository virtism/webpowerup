...
class Menus_Model extends CI_Model{
	...
	function updateMenu(){
		...
		//echo $intNumItems;exit;
		$menu_sort_array = Array();
	  
		$i=1;
		$j=0;
		$menu_sort_array_raw = array_slice($_POST, 11);
		
		foreach( $menu_sort_array_raw as $key => $value )
		{
		...
		if($intNumItems>0)
		{
			...
			//echo "total count ---".$_POST["numItems"]."<br>";
			$usman = 1;	
			
			for( $i=1; $i<=$intNumItems; $i++ )
			{
				
				if(isset($menu_sort_array['txtItemName'.$i]) && isset($menu_sort_array['lstItemPage'.$i]) && isset($menu_sort_array['rdoItemPublished'.$i]))
				{
					if($menu_sort_array['txtItemName'.$i]!='' && $menu_sort_array['lstItemPage'.$i]!='0' && $menu_sort_array['rdoItemPublished'.$i]!='')
					{
					 
					 $usman++;
					 	
					 
						$strItemName = $menu_sort_array['txtItemName'.$i];
						$strItemPage = $menu_sort_array['lstItemPage'.$i];
						$strPublished = $menu_sort_array['rdoItemPublished'.$i];    
						if(!empty($menu_sort_array['_lstItemPage'.$i]))
						{
							$strCustomLink = $menu_sort_array['_lstItemPage'.$i];  
							
						}else if(!empty($menu_sort_array['value_clink_'.$i]))
						{
							$strCustomLink = $menu_sort_array['value_clink_'.$i];
						}else
						{
							$strCustomLink = "";
						}					
						...
						$this->db->query("INSERT INTO menuitems(item_name, item_published, item_status,item_link) VALUES(".$this->db->escape($strItemName).",  ".$this->db->escape($strPublished).", 'Active','".$strCustomLink."')");
						
						$intItemId = $this->db->insert_id();
					 //   echo "INSERT INTO menuitems(item_name, item_published, item_status,item_link) VALUES(".$this->db->escape($strItemName).",  ".$this->db->escape($strPublished).", 'Active','".$strCustomLink."')<br>";
				   //     echo "INSERT INTO menus_menuitems_xref(menu_id, item_id) VALUES(".$this->db->escape($id).", ".$this->db->escape($intItemId).")<br>";
					 //   echo "INSERT INTO menuitems_pages_xref(item_id, page_id) VALUES(".$this->db->escape($intItemId).", ".$this->db->escape($strItemPage).")<br>";
					//    exit;
	...
	function updateMainMenu()
	{
		...
		$menu_sort_array = array_slice($_POST, 3);
		
		$k=1;
		$reset=1;
		$menu_sorted_array = array();
		$temp_array = array();
		//echo "<pre>";
		foreach($menu_sort_array as $key => $value)
		{ 
			$newKey = substr($key,0,-1); 
			if(is_numeric(substr($newKey, -1)))
			{
				$newKey = substr($newKey,0,-1);
			}
			$temp_array[$newKey] = $value;	
			if($k == 6) 
			{
				
				$menu_sorted_array[] = $temp_array;
				$k = $reset;
				unset($temp_array);
				$temp_array = array();
		...
		// echo "<pre>";	print_r($menu_sort_array);	echo "</pre>****";
		// echo "<pre>";	print_r($menu_sorted_array);	echo "</pre>"; die();
		//print_r($menu_sorted_array);
		//exit;
		$this->db->query("DELETE FROM menuitems_pages_xref WHERE item_id IN(SELECT item_id FROM menus_menuitems_xref WHERE menu_id=".$this->db->escape($id).")");
		...
		if($intNumItems>0)
		{
			...
			// echo "<pre>";	print_r($menu_sort_array);	echo "</pre>********";
			// echo "<pre>";	print_r($menu_sorted_array);	echo "</pre>"; die();
			foreach($menu_sorted_array as $key => $value)
			{
						$menuItem = $value;
						
						$strItemName = $menuItem['txtItemName'];
						$strItemPage = $menuItem['lstItemPage'];
						$strPublished = $menuItem['rdoItemPublished'];    
						$parent_id = $menuItem['parent_id']; 
						$item_link = $menuItem['_lstItemPage'];
						
						$item_target = $menuItem['menuTarget'];
						
						$display_order = $key; 
						
						
						
						$Q = "INSERT INTO menuitems
						(
						item_name, 
						item_published, 
						item_status, 
						...
						VALUES(
						".$this->db->escape($strItemName).",
						".$this->db->escape($strPublished).",
						'Active',
						".$this->db->escape($item_link).",
						'".$item_target."',
						".$key." ) ";
						$this->db->query($Q);
						$intItemId = $this->db->insert_id();
...
*/
	if($access_level == "Everyone" || $access_level == "" )
	{
		...
		return $this->db->query("SELECT mi.item_id, item_name, item_status, item_published, item_link,access_level  FROM menuitems mi JOIN menus_menuitems_xref mix ON mi.item_id=mix.item_id JOIN menus mn ON mn.menu_id=mix.menu_id WHERE mn.menu_id=".$this->db->escape($menu_id)." AND item_status='Active' AND item_published='Yes' ORDER BY display_order ASC");        
	}
	//return true;
		...
		if($intNumItems>0)
		{
			...
			for($i=1; $i<=$intNumItems;$i++)
			{
				if($this->input->post("txtItemName".$i)!="" && $this->input->post("lstItemPage".$i)!="" && $this->input->post("rdoItemPublished".$i)!="" && $this->input->post("menuTarget".$i)!=""){
					...
					$this->db->query("INSERT INTO menuitems(item_name, item_published, item_status, item_link, item_target, display_order) VALUES(".$this->db->escape($strItemName).",  ".$this->db->escape($strPublished).", 'Active' , ".$this->db->escape($strCustomLink)." , ".$this->db->escape($menuTarget)." ,'$k')");
