<?php
  class Testimonial_Reviews_Management extends CI_Controller{
      ...
      { 
           ...
           if ($this->input->post('deleteAll'))   
            {
                 ...
                 $totalcheckbox= $this->input->post('totalcheckbox');
                 for($i=1;$i<=$totalcheckbox;$i++)
                 {
                       $chkbox = "check_list".$i;
                       if( $this->input->post($chkbox))
                       {
                         $val = $this->input->post($chkbox); 
                         
                         $this->Testimonial_Model->delete_review_soft($val); 
                       //  $this->Testimonial_Model->delete_review_hard($val);  
            ...
            }else if ($this->input->post('approve'))   
            {
                 ...
                 for($i=1;$i<=$totalcheckbox;$i++)
                 {
                       ...
                       if( $this->input->post($chkbox))
                       {
                         $val = $this->input->post($chkbox); 
                         //$this->Testimonial_Model->delete_review_soft($val);  
                         $this->Testimonial_Model->approve_review($val); 
                        // redirect(base_url().'index.php/Testimonial_Reviews_Management/index/'.$page_id);   
                       }
                 }     
            }
       ...
       $show_data['view_all_records'] = $this->Testimonial_Model->show_selected_testimonies($page_id);
       $show_data['reviews'] = $this->Testimonial_Model->show_all_reviews($page_id); 
       $this->load->view('Testimonial_Reviews_Management_View',$show_data);
...
//       $data['news_date_sent'] = $data_testimonial['news_date_sent'];
    ...
    // To all delete business review 
      function approve_unapprove_reviews ()
      {
        ...
        $this->Testimonial_Model->change_status($id);      
        //redirect(base_url().'index.php/Testimonial_Management');  
        redirect(base_url().'index.php/Testimonial_Reviews_Management/index/'.$page_id); 
      }
...
//             echo  $data_tii."<>>>>>>>>>>>>";
            ...
            // redirect(base_url().'index.php/Testimonial_Management/view_testimonial/'.$data[5]);  
