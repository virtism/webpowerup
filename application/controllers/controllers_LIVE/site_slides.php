        ...
        for($i=1; $i<= intval($numImages); $i++)
        {
            ...
            if($_FILES['slide_image'.$i]['tmp_name']!='')
            {
              ...
              //  $this->image_lib->clear(); 
                ...
                $this->upload->do_upload("slide_image".$i);              
                $file_info = $this->upload->data();
                $this->Slideshow_Model->save_slideshow_image_info($slide_id, $slide_image, $slide_image_url,$slide_title,$slide_description);
    ...
    function edit_slider_info($site_id, $slide_id)
    {
        ...
        for($i=1; $i<= intval($numImages); $i++)
        {
            ...
            if(isset($_FILES['slide_image'.$i]['tmp_name']) && $_FILES['slide_image'.$i]['tmp_name']!='')
            {
                ...
                $slide_image = $time.$_FILES['slide_image'.$i]['name']; 
                $slide_image_url = $_POST['slide_image_url'.$i];
                $slide_title_ = $_POST['slide_title'.$i];
                $slide_descrp_ = $_POST['slide_description'.$i];
                ...
                $this->upload->do_upload("slide_image".$i);              
                $file_info = $this->upload->data();
                $this->Slideshow_Model->save_slideshow_image_info($slide_id, $slide_image, $slide_image_url,$slide_title_ , $slide_descrp_);
            }
        }
