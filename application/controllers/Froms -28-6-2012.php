           ...
           //echo $total.":: ToTal >>>>>>>>>>>>";  exit();
            ...
            {   //  is_array($items)
                ...
                if(!empty($item['value']) && !empty($item['name']))
				{
					...
					if($form_values_arr[$i]['value']=='on')
					{
					
						$form_values_arr[$i]['value'] = 'Yes';
					}
					$msg_bdy .=  $form_values_arr[$i]['name'].' : '.$form_values_arr[$i]['value']."\n";
				}
				else if(isset($_REQUEST['form_id']) && $flag == 0)
				{
					if(isset($_REQUEST[$_REQUEST['form_id']]))
					{
						$msg_bdy .= $_REQUEST[$_REQUEST['form_id']]." : Yes\n";
						$flag = 1;
					}			
				}				
                $i++;
            ...
            //echo '<pre>'; print_r($form_values_arr); exit();
            		...
            		$message = "";
                    $message .=  "This email send via WebpowerUp Please Dont Reply it"."\r\n";
                    $message .=  $msg_bdy;
                    ...
                    $this->email->subject($form_detail['form_title'].' | Registration Form Mail');
                    $this->email->message($message);
                    $this->email->send();
					$headers = 'From: form@webpowerup.com';
					mail($form_detail['form_email_to'], $form_detail['form_title'], $message, $headers);
