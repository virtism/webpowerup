...
//		 echo "<pre>";
	...
	$image_path = base_url().'media/ckeditor_uploads/'.$result[0]['user_login'].'_'.$result[0]['user_id'].'/galleries/';
			...
			if(isset($gallery_data[$i]['folder_name']) && $gallery_data[$i]['folder_name'] == 'imageflow')
			{
				...
				for($j = 0; $j<count($gallery_images); $j++)
				{
					...
					$gallery_html .= '<img src="'.$image_path.$image_name.'" longdesc="'.base_url().'galleries/'.$image_name.'"  alt="Image '.$j.'" />';
				}
			...
			else if(isset($gallery_data[$i]['folder_name']) && $gallery_data[$i]['folder_name'] == 'simple_thumbs')
			{
				...
				for($j = 0; $j<count($gallery_images); $j++)
				{
					...
					$gallery_html .= '<li><a  target="'.$target.'" href="'.$url.'"><img src="'.$image_path.$image_name.'" alt="Image '.$j.'" /></a></li>';
				}
	...
	{   
		...
		$video_gallery_data = $this->Video_Gallery_Model->get_all_gallery_data($page_id);
