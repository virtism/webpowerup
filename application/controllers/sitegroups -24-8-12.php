	...
	*/
...

	{
...

		{
			...
			// echo "<pre>";	print_r($data['data'][0]['group_page_id']);		echo "</pre>";
...

	{
		...
		for($i=0; $i<$fields_count; $i++)
		{
			if ($fields_data[$i]['field_type'] == 'Check Boxes')
			{
			 	...
			 	if($fields_data[$i]['field_required'] == "Yes")
				{
				 	$data  .= "<tr><td>".$fields_data[$i]['field_title']."</td><td>
					<input class='required_custom_field' type='checkbox' name=\"field[".$i."][value]\" />
					<input id='".$fields_data[$i]['id']."' type='hidden' value='".$fields_data[$i]['field_name']."' name=\"field[".$i."][name]\" ></td></tr>";
				}
				else
				{
				 	$data  .= "<tr><td>".$fields_data[$i]['field_title']."</td><td>
					<input  type='checkbox' name=\"field[".$i."][value]\" />
					<input id='".$fields_data[$i]['id']."' type='hidden' value='".$fields_data[$i]['field_name']."' name=\"field[".$i."][name]\" ></td></tr>";
				}
			 ...
			 else if($fields_data[$i]['field_type'] == 'Radio Buttons')
			 {
				...
				{
					$data  .= "<tr>
							<td>".$fields_data[$i]['field_title']."</td><td>
							<input type='radio' class='required_custom_field' value='yes' name=\"field[".$i."][value]\" />
							<input id='".$fields_data[$i]['id']."' type='hidden' value='".$fields_data[$i]['field_name']."' name=\"field[".$i."][name]\" ></td></tr>";
				}
				else
				{
					$data  .= "<tr><td>".$fields_data[$i]['field_title']."</td><td>
					<input type='radio' value='yes' name=\"field[".$i."][value]\" />
					<input id='".$fields_data[$i]['id']."' type='hidden' value='".$fields_data[$i]['field_name']."' name=\"field[".$i."][name]\" ></td></tr>";
				}
			 ...
			 else if($fields_data[$i]['field_type'] == 'Single-Line Text' )
			 {
				...
				{
					$data  .= "<tr><td>".$fields_data[$i]['field_title']."</td><td>
					<input type='text' class='required_custom_field' name=\"field[".$i."][value]\" />
					<input id='".$fields_data[$i]['id']."' type='hidden' value='".$fields_data[$i]['field_name']."' name=\"field[".$i."][name]\" ></td></tr>"; 
				}
				else
				{
					$data  .= "<tr><td>".$fields_data[$i]['field_title']."</td><td>
					<input type='text' name=\"field[".$i."][value]\" />
					<input id='".$fields_data[$i]['id']."' type='hidden' value='".$fields_data[$i]['field_name']."' name=\"field[".$i."][name]\" ></td></tr>";   
				}
			 ...
			 else if ($fields_data[$i]['field_type'] == 'Multi-Line Text')
			 {
				...
				{
					$data  .= "<tr><td>".$fields_data[$i]['field_title']."</td><td>
					<textarea class='required_custom_field' name=\"field[".$i."][value]\" ></textarea>
					<input id='".$fields_data[$i]['id']."' type='hidden' value='".$fields_data[$i]['field_name']."' name=\"field[".$i."][name]\" ></td></tr>";	
				}
				else
				{
					$data  .= "<tr><td>".$fields_data[$i]['field_title']."</td><td><textarea name=\"field[".$i."][value]\" ></textarea>
					<input id='".$fields_data[$i]['id']."' type='hidden' value='".$fields_data[$i]['field_name']."' name=\"field[".$i."][name]\" ></td></tr>";	
				}
			 }
		}		
			...
			<div class="section" id="paypal_row">
				<div>
					<form action="<?=$this->paypal_lib->paypal_url?>" method="post" id="payPalRegForm" >
						...
						<input type="hidden" name="currency_code" value="USD">
						<input type="hidden" name="rm" value="2">   
						<input type="hidden" name="return" value="<?=base_url().index_page()?>MyAccount/payment_process/<?php echo $site_id."/paid"; ?>" >
						<input type="hidden" name="cancel_return" value="<?=base_url().index_page()?>MyAccount/register/<?php echo $site_id; ?>">
						<input type="hidden" name="notify_url" value="<?=base_url().index_page()?>paypal/ipn">      
						<input type="hidden" name="item_name" value="Charged Amount for Plan <?=$check_group_paid[0]["group_name"]?>">
						<!--  <input type="hidden" name="item_number" value="786"> -->
						<input type="hidden" name="quantity" value="1">
