...
class PagesController extends CI_Controller{
	...
	function basic_info($site_id, $private='', $private_user_id = 0)
	{
		//DebugBreak();
        $this->checkLogin();
		$info_link = current_url();
		$this->session->set_userdata("info_link", $info_link); 
		...
		$this->breadcrumb->add_crumb('Create Page' ); 
	...
	function create_page()
	{
		...
		if($page_type == "private")
		{
            ...
            {
                $_SESSION['members'] = $this->input->post('members');
	...
	{
		$menus_link = current_url();
		$this->session->set_userdata("menus_link", $menus_link); 
		//confirm user is logged in 
		$this->breadcrumb->clear();
		$this->breadcrumb->add_crumb('Main', $this->session->userdata("mainPage_link") );
		$this->breadcrumb->add_crumb('Dashboard', $this->session->userdata("dashboard_link") ); 
		$this->breadcrumb->add_crumb('Create Page', $this->session->userdata("info_link") );
		$this->breadcrumb->add_crumb('Menu & Access' ); 
	...
	{   
		...
		$editor_link = base_url().$link;
		$this->session->set_userdata("editor_link", $editor_link); 
		
		$this->breadcrumb->clear();
		$this->breadcrumb->add_crumb('Main', $this->session->userdata("mainPage_link") );
		$this->breadcrumb->add_crumb('Dashboard', $this->session->userdata("dashboard_link") ); 
		$this->breadcrumb->add_crumb('Create Page', $this->session->userdata("info_link") );
		$this->breadcrumb->add_crumb('Menu & Access', $this->session->userdata("menus_link") );
		$this->breadcrumb->add_crumb('Editor Board' ); 
		//$data['site_id'] =  $this->input->post("site_id");
		$data['site_id'] =  $site_id;
		$data['page_id'] =  $page_id;
	...
	{   
		$link = substr(uri_string(),1);
		$layout_link = base_url().$link;
		$this->session->set_userdata("layout_link", $layout_link); 
		...
		$this->breadcrumb->add_crumb('Create Page', $this->session->userdata("info_link") );
		$this->breadcrumb->add_crumb('Menu & Access', $this->session->userdata("menus_link") );
		$this->breadcrumb->add_crumb('Editor Board', $this->session->userdata("editor_link") );
		$this->breadcrumb->add_crumb('Layout Description' );
		$this->checkLogin(); 
		$data['site_id'] =  $site_id;
		$data['page_id'] =  $page_id;
		$data['item_id'] =  $item_id;   
		$data['template_page_names'] = $this->Pages_Model->get_page_template_options($site_id);
	...
	function layout_desc_edit($site_id, $page_id, $item_id)
    {   
        ...
        $this->template->add_js('js/jscolor/jscolor.js');
         ...
         $editlayot = array();
           foreach($data['pages_list']->result_array() as $row_pages_list)
           {  
               if($page_id == $row_pages_list['page_id']) 
               {
                  $editlayot[] = $row_pages_list; 
                  break;
               }
               
           }
         // echo '<pre>';print_r($editlayot);exit;
          $data['editlayot'] =   $editlayot;
       
        $this->template->write_view('content','page/create/layout_desc_edit', $data);
        $this->template->render();   
        //echo $item_id;
        //$this->load->view("page/create/layout_desc");   
    }
    function upadte_upload_page_layout_desc()
    {
       ...
       // DebugBreak();
        ...
        $pathHeader = $path.'headers/';
        $path_page_BG = $path.'background/';
        $pathHeader_BG = $path.'headers_background/';
        
        if(!is_dir($pathHeader))
        {
            mkdir($pathHeader,0777);
        }
        else if (!is_dir($path_page_BG))
        {
            mkdir($path_page_BG,0777);
        }
        else if (!is_dir($pathHeader_BG))
        {
            mkdir($pathHeader_BG,0777);
        }
        
        
        if(isset($_REQUEST['page_template_options']) && $_REQUEST['page_template_options']==0)
        {
        ...
        
                {
        ...
        
                    {
                        ...
                        if($_FILES['header_image_'.$i]['tmp_name']!="")
                        {
                            ...
                            if ( !$this->upload->do_upload("header_image_".$i) ) 
                            {
                ...
                if($header_background == "Image" && $_FILES["header_background_image"]["tmp_name"]!=""){            
                    ...
                    $config['upload_path'] = $pathHeader_BG;            
                ...
                if($page_background == "Other" && $_FILES["background_image"]["tmp_name"]!=""){            
                    ...
                    $config['upload_path'] = $path_page_BG;            
                ...
                $this->Pages_Model->update_upload_page_layout_desc();
        }
        else
        {
            $this->Pages_Model->set_template_layout_page();
        }
	...
	function showPageInfo($site_id, $id){
		...
		echo $data["page_status"] = $row["page_status"];
	...
	function success($page_id, $site_id)
	{
		...
		$data["page_id"] = $page_id;
		$data["site_id"] = $site_id;  
		$this->template->write_view('content','page/create/create_page_success', $data);
		$this->template->render();
	...
	function searchPage($site_id, $from){
		...
		$this->template->write_view('content','page/viewPages', $data);
		$this->template->render();    
	...
	function index($site_id, $from = 1){ 
	 	$link = substr(uri_string(),1);
		$pages_link = base_url().$link;
		$this->session->set_userdata("pages_link", $pages_link); 
		
		$this->breadcrumb->clear();
		$this->breadcrumb->add_crumb('Main', $this->session->userdata("mainPage_link") );
		$this->breadcrumb->add_crumb('Dashboard', $this->session->userdata("dashboard_link") ); 
		$this->breadcrumb->add_crumb('Manage Page');
       ...
       /* echo "<pre>";
		...
		$config = array(
		 ...
		 //echo "<pre>";print_r($data["records"]->result_array());exit;
		/*$config['uri_segment'] = 4;        
    ...
    function private_pages($site_id, $from, $private_user_id=''){ 

	 	
		//echo $private_user_id;exit;
		$link = substr(uri_string(),1);
		$pages_link = base_url().$link;
		$this->session->set_userdata("pages_link", $pages_link); 
		...
		$data['site_id'] = $site_id;
		if(isset($private_user_id) && $private_user_id!='' && $private_user_id!=0)
		{
			$data["records"] = $this->Pages_Model->get_user_manage_private_pages($site_id, $private_user_id);
		}
		else
		{
			$data["records"] = $this->Pages_Model->showPrivatePages($from, $data["pageLimit"], $site_id);
		}
		/*echo "<pre>";
		print_r($data["records"]);
		exit;*/
		//$data["privar_pages_per_user"] = $this->Pages_Model->get_user_private_pages($site_id,$user_id)
		$data["pages_list"] = $this->Pages_Model->getPrivatePages($site_id);
		$data["numRecords"] = $data["records"]->num_rows();
		$data["from"] = $from;
		$data["totalRecords"] = $this->Pages_Model->totalPrivatePages($site_id); 
		$config = array(

			'uri_segment' => 4,

			'base_url' => base_url().index_page().'pagesController/private_pages/'.$site_id."/",
		...
		print_r($data);
		exit;*/
		$this->template->write_view('content','page/viewPrivatePages', $data);
		$this->template->render();
		//$this->load->view("viewPages", $data);
	}
	...
	*/
	...
	function updatePageContent_oldoooo($site_id, $page_id)
	{
  ...
  //      redirect('page_editor/index/'.$site_id.'/'.$page_id);
		redirect('site_preview/page/'.$site_id.'/'.$page_id);
	}
	function updatePageContent($site_id, $page_id)
	{
		...
		//go back to page in edit mode called page editor
		$site_detail = $this->SitesModel->get_site_by_id($site_id);//print_r($site_detail[0]['site_url']);exit;
		/*
		redirect('http://'.$site_detail[0]['site_url'].'.webpowerup.com/site_preview/page/'.$site_id.'/'.$page_id);
		*/
		redirect(site_url()."site_preview/page/".$site_id.'/'.$page_id);
		//redirect('page_editor/index/'.$site_id.'/'.$page_id);
	}
	function updatePageStatus()
	{
		if(isset($_REQUEST['site_id'])&&isset($_REQUEST['page_id']))
		{
		//echo $_REQUEST['site_id']."-----".$_REQUEST['page_id'];
		$this->Pages_Model->publish_unpublish_page($_REQUEST['site_id'],$_REQUEST['page_id'],$_REQUEST['status']);
		}
	}
	
	function publich_unpublish_title()
	{
		if(isset($_REQUEST['site_id']) && isset($_REQUEST['page_id']))
		{
			$this->Pages_Model->publich_unpublish_title($_REQUEST['site_id'],$_REQUEST['page_id'],$_REQUEST['status']);
		}
	}
	function get_groups_members()
	{
		
		$members = $this->Groups_Model->get_multiple_group_members();
		
			if($members)
			{
				//$html = "<select name=\"members[]\" multiple=\"multiple\" style=\"opacity:1;\">";
				$html = "<div style=' position:relative;' ><select name=\"members[]\"   size=\"1\" style=\"width:360px;\">";
				foreach($members as $customer_id => $login )
				{
					$html .= "<option value=\"".$customer_id."\">".$login."</option>";
	...
	function get_groups_members_selected()
	{
		$page_users = $this->input->post("page_users");
		
		$page_users = explode(",",$page_users);
		$members = $this->Groups_Model->get_multiple_group_members();
			...
			if($members)
			{
				...
				{
					$selected = "";
					if(in_array($customer_id,$page_users))
					{
						$selected = " selected=\"selected\" ";
					}
					
					$html .= "<option ".$selected." value=\"".$customer_id."\">".$login."</option>";
