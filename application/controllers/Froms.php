    ...
    var $temp_name;
      ...
      { 
        ...
        if($this->input->post('action') == 'done')
        {
		   ...
		   $site_info 		= $this->auth->get_site_admin_by_site_id($form_detail['site_id']);
		   $site_admin_info = $this->auth->get_admin_details_by_id($site_info['user_id']);
		 /* echo '<pre>';
		   //print_r($_REQUEST);
		   print_r($form_detail);
		   print_r($_SESSION);
           ...
           //echo $total.":: ToTal >>>>>>>>>>>>";  exit();
			...
			{ 	
				...
				//	echo $form_detail['form_email_to'];
                    ...
                    $message .= "<a href='".$site_info['domain']."' >".$site_info['domain']."</a>";
