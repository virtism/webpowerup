...
class Products_Management extends CI_Controller {
   ...
   function index(){
		...
		$this->session->set_userdata("product_link", $product_link); 
		...
		$this->breadcrumb->add_crumb('Manage products' );
	    // Setting variables
    ...
    function create($invoice_create_time=0){
        ...
        $this->breadcrumb->add_crumb('Main', $this->session->userdata("mainPage_link") );
		$this->breadcrumb->add_crumb('Dashboard', $this->session->userdata("dashboard_link") ); 
		$this->breadcrumb->add_crumb('Manage products', $this->session->userdata("product_link") ); 
		$this->breadcrumb->add_crumb('Add');
