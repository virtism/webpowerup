...
class webinar_controller extends CI_Controller {
	...
	function new_webinar($site_id)
	{
		...
		/* $count = $this->customers_model->getAllCustomersCountBySiteID($site_id);
		 ...
		 $data['groups_users'] = $this->Webinar_Model->get_site_gropus_customer_by_site_id($site_id);	
		...
		//echo "<pre>";print_r($data['times_zones']);exit;
	...
	function do_edit_webinar($webinar_id)
	{
		...
		if(isset($_POST["action"]) && $_POST["action"] == "do_edit")
		{
			...
			$webinar_rid = $this->Webinar_Model->save_edit_existing_users($webinar_id);
			
			$data['selected_users'] = $this->Webinar_Model->get_all_meeting_user($webinar_id);
		...
		$data['menus'] = $this->Menus_Model->getAllMenus($data['site_id']);
		// $data['ck_data'] = $this->ck_data;
		$data['groups_users'] = $this->Webinar_Model->get_site_gropus_customer_by_site_id($_SESSION["site_id"]);
		$data['selected_users'] = $this->Webinar_Model->get_all_meeting_user($webinar_id);
		$data['options']= $this->Webinar_Model->get_form_fields_options($webinar_id);
		$data['fields']= $webinar_data[0]['fields'];//$this->Webinar_Model->get_form_fields($webinar_id);
		$data['times_zones'] = $this->Webinar_Model->getAllTimeZone();
		$data['is_edit'] = "true";	
