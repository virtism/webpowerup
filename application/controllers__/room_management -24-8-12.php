...
if(!session_start()){
  ...
  class Room_Management extends CI_Controller{
      ...
      function __construct()
      {
		...
		function create_new_room()
		{
			...
			if(isset($action) && !empty($action))
			{
				...
				$email1 = $this->Room_Model->get_all_site_gropus_user_email($room_data['room_rid']);
		...
		/*if(isset($room_data[0]->customers)&&!empty($room_data[0]->customers))
			...
			exit;*/
			//Get All Email Ids Of Customers If Selected
			$email1 = $this->Room_Model->get_all_existing_user_email($room_id);
		...
		function send_mail($customers, $noncustomers, $sub, $room_rid, $mess, $room_id)
		{
				...
				{
					//$id_messg = '';
					$body = $mess."<br/><a href='http://webpowerup.com/broadcast/GWSWhiteboard.html#roomID=".$room_rid."&attendee_id=".($mail['customer_id']*2)."'>Join Meeting</a>";					
					//$body .= $id_messg;
	  ...
	  function save_edit_room($room_id)
	  {
			...
			$room_rid = $this->Room_Model->save_edit_existing_users($room_id);
		
			$save = $this->Room_Model->save_edit_room($room_data, $room_id);
			if(isset($action) && !empty($action))
			{
				//Get All Email Ids Of Customers If Selected
				$email1 = $this->Room_Model->get_all_site_gropus_user_email($room_rid);							
				$email2 = explode(",", $room_data['non_group']);
				$emails = array_merge($email1,$email2);
				$this->send_mail($email1, $email2, $room_data['topic'],$room_rid, $message, $save);
				$this->send_mail_presenter($_SESSION['user_info']['user_id'], $_SESSION['user_info']['user_email'], $room_rid, $room_data['topic'], $message, $save);
			}
   ...
   //    $data['page'] = $this->PagesModel->get_pages_dropdown($this->site_id); 
	...
	function get_all_users_groups()
	{
		...
		$data['groups_users'] = $this->Room_Model->get_site_gropus_customer_by_site_id($this->site_id);		
		$this->load->view('room_all_groups_users',$data);
	}
	function save_existing_user()
	{
		
				
		$this->Room_Model->save_existing_users();		
		//$this->load->view('room_all_groups_users',$data);
	}
}
?>