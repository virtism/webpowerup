  ...
  class Room_Management extends CI_Controller{
      ...
      function __construct()
      {
		...
		function send_invitation($room_id)
		{
			...
			$message = "<table width='600' border='0'><tr><td>Meeting Name: </td><td>".$room_data[0]->name."</td></tr><tr><td>Topic Name:</td><td>".$room_data[0]->topic."</td></tr><tr><td>Start Date:</td><td>".$room_data[0]->reg_date_start."</td></tr><tr><td>Message</td><td>".$room_data[0]->message."</td></tr></table>";
		...
		function send_mail($customers, $noncustomers, $sub, $room_rid, $mess, $room_id)
		{
				...
				foreach($customers as $id => $mail)
				{
					...
					//$id_messg = '';
					$body = $mess."<br/><a href='http://webpowerup.com/broadcast/GWSWhiteboard.html#roomID=".$room_rid."&attendee_id=".(2*$id)."'>Join Meeting</a>";
				...
				{
					$body = $mess."<br/><a href='http://webpowerup.com/broadcast/GWSWhiteboard.html#roomID=".$room_rid."&attendee_id=private'>Join Meeting</a>";
					$this->email->from('join_meeting@webpowerup.com', 'WebpowerUp');
					$this->email->subject($subject);
					$this->email->message($body);	
					$this->email->to($mail); 
		...
		function send_mail_presenter($presenter_id, $presenter_mail, $room_rid, $sub, $mess, $room_id)
		{
				...
				$config['wordwrap'] = TRUE;
				$this->email->initialize($config);	
				$body = $mess."<br/><a href='http://webpowerup.com/broadcast/GWSWhiteboard.html#roomID=".$room_rid."&attendee_id=".$presenter_id."'>Join Meeting</a>";
				$this->email->from('join_meeting@webpowerup.com', 'WebpowerUp');
