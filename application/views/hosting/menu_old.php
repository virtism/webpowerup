       ...
       if(!empty($other_top_navigation) || count ($other_top_navigation) > 0 )
           for($a=0; $a<count($other_top_navigation); $a++)
           {
               if($mode=='edit')
                ...
                {
                    $strLink = base_url().'index.php/'.$other_top_navigation[$a]['link'].'/';
                }
...
<!--<div class="nevibuttona">
       ...
       for($i=0; $i<sizeof($adv_menu); $i++)
        {
            ...
            if($boolDisplay == TRUE)
            {
                if($mode=='edit')
                {
                    ...
                    <a class="edit_menu" href="'.base_url().'index.php/page_editor/editMenuInfo/'.$site_id.'/'.$adv_menu[$i]['menu_id'].'"><img src="'.base_url().'images/edit_icon.jpg" style="border:none;background:none;" /></a>
                    ';       
                }
        ?>
            <li>
                ...
                <ul>
                    ...
                    for($j=0; $j<sizeof($adv_menu[$i]['sub_menu']); $j++)
                    {
                        ...
                        {
                            $strLink = base_url()."index.php/site_preview/page/".$site_id."/".$adv_menu[$i]['sub_menu'][$j]['page_id'];
                        }
                    ?>
    ...
    {
        echo '<a class="edit_menu" href="'.base_url().'index.php/page_editor/editMenuInfo/'.$site_id.'/'.$menu_id.'" style="font-size:10px;">Edit</a>';
    }
    ?>
