...
{ 
		...
		if(!$is_user_paid )
		{
			...
			if(!$this->session->userdata('payment_request'))
			{
			...
			{
				echo '<div style="color:#009900">'.$this->session->userdata('payment_request').'</div>';
				$this->session->unset_userdata('payment_request');
