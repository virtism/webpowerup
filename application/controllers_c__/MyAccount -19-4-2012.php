		...
		if ($this->input->post('action') && $this->input->post('action') == 'do_login'){
...

				{
...

					{
						...
						redirect(base_url().index_page().'site_preview/page/'.$this->site_id.'/'.$group_page);        
...

  {
		...
		exit;*/
		 if ($this->input->post('action') && $this->input->post('action') == 'addCustomer'){
				...
				if(isset($_REQUEST['paid_group']))

				{

					if(empty($_REQUEST['pending_membershipid']))

					{

						redirect(base_url().index_page().'MyAccount/register');
				...
				if(isset($_POST['get_site_id']))
				{
					$this->site_id	= $_POST['get_site_id'];			
					$_POST['passconf'] = rand(100000,9999999);
...
*/	
					...
					$message .= "Thank you for Pre-Registering!\n\nPre-Registration gives you first look at our very powerful and complete set of web tools designed specifically to make your business more effective and productive.\n\nBy being one of the first you will get 60 days to try out all our tools for free.\n\nIf you have any questions, let us know.\n\n#Login Mail: ".$mail."\n\n#Password: ".$password."\n\nThank you for your interest in WebPowerUp!\n\nWebPowerUp! Team";
	 $message .= '  http://webpowerup.net/';
				   ...
				   // $this->email->bcc('them@their-example.com');
					$this->email->subject('WebPowerUp! Pre-Registration Confirmation');
					$this->email->message($message);
					$this->email->send();
				   // echo $this->email->print_debugger();
					//echo base_url().index_page();exit;
					if(isset($_REQUEST['get_site_id']))
					{
						
						echo "1";
						//$this->config->item('c_customer') = 1;
...
*/
	...
	$mail =   mail($to, "Note from the Contact Form", $message);
	 if ($mail)
	 {
	    echo 'sent';
	 }else
	...
	/*
		...
		$data;
		  ...
		  */
			...
			$temp_name1 =  $this->my_template_menu->set_get_template($this->site_id);

			$data['logo_image'] = $this->Site_Model->get_logo_image($this->site_id);

			$this->template->write_view('logo',$temp_name1.'/logo', $data);
