...
class MenusController extends CI_Controller{
    ...
    function updateMenu(){
        ...
        */
        ...
        //echo $data['site_id'];exit();
        if($_POST){  
            //perform form validation if form is posted
            //sets form validation rules
            $this->form_validation->set_error_delimiters('<span>', '</span>');
            $this->form_validation->set_rules('txtName', 'Menu Name', 'required');
            if($this->input->post('rdoPublished')=="Schedule"){
                $this->form_validation->set_rules('startDate', 'Start Date', 'required');
                $this->form_validation->set_rules('endDate', 'End Date', 'required|callback_scheduled_dates');                         
            }
            if($this->input->post('rdoPages')=="Other"){
                $this->form_validation->set_rules('lstPages[]', 'Only on Pages', 'required');                
            }
            if($this->input->post('rdoRights')=="Other"){
                $this->form_validation->set_rules('lstRoles[]', 'Only for Roles', 'required');                
            }
            //applied form validation rules
            if ($this->form_validation->run() == FALSE){
                //form validation doesn't pass
                $data['id'] = $this->input->post("id");
                   
                $data['pages'] = $this->Menus_Model->getPages($site_id);
                $data['roles'] = $this->Menus_Model->getRoles();
                $data["menu_name"] = $this->input->post("txtName");
                $data["menu_position"] = $this->input->post("rdoPosition");
                $data["menu_published"] = $this->input->post("rdoPublished");
                $data["numItems"] = $this->input->post("numItems");
                if($data["menu_published"]=="Schedule"){            
                    $data["menu_start"] = $this->input->post("startDate");
                    $data["menu_end"] = $this->input->post("endDate");
                }
                else{
                    $data["menu_start"] = "";
                    $data["menu_end"] = "";
                } 
                $data["menu_pages"] = $this->input->post("rdoPages");
                $data["menu_access"] = $this->input->post("rdoRights");
    ...
    function addMenu($site_id){     
      //  echo "<pre>";
       // print_r($_POST);
        //echo $this->input->post('rdoPages');
       //exit;
       //confirm user is logged in
        $this->checkLogin();
        
        if($this->input->post('rdoPages')){
            ...
            if ($this->form_validation->run() == FALSE){
                ...
                //prepare data for addMenuView.php(view)
                $data['site_id'] = $site_id;
                $data['pages'] = $this->Menus_Model->getPages($site_id);
                $data['roles'] = $this->Menus_Model->getRoles();
                $data["fieldsData"] = $this->input->post();
                //fill content region of template with addMenuView.php and $data 
                ...
                //form validation passed
                //create / add information menu in db
                $this->Menus_Model->addMenu($this->input->post());
                //redirect user to menus management screen
                redirect("menusController/index/".$site_id."/0");
            }           
        }          
    }
    ...
    function index($site_id, $from){  
        ...
        $config = array(
            'uri_segment' => 4,
            'base_url' => base_url().'index.php/menusController/index/'.$site_id."/",
        ...
        $config['base_url'] = base_url().'index.php/menusController/index/'.$site_id."/";
