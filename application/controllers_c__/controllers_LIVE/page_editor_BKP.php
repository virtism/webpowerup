...
class Page_editor extends CI_Controller
{
	...
	*/
	...
	{   
		...
		$content .= '<div><h2>'.$page_title.' <a style="font-size:10px;" class="edit_title" href="'.base_url().'index.php/page_editor/editPageTitle/'.$site_id.'/'.$page_id.'">Edit Title</a></h2></div>';
		...
		if($result->num_rows()>0)
		{
			foreach($result->result_array() as $row)
			{
				...
				if($row['type']=="image")
				{
					...
					<div id="content_area_'.$i.'" style="border:1px dotted #CCCCCC; margin-bottom:3px;">
					<div>
					<a id="image_'.$i.'" href="'.base_url().'index.php/media/index/'.$i.'">
		...
		*/
		...
		$content = '<div>
		<h2>'.$page_title.'
		<a class="edit_title" href="'.base_url().'index.php/page_editor/editPageTitle/'.$site_id.'/'.$page_id.'" style="font-size:10px;">
		Edit Title
		</a></h2>
		</div>';
			...
			foreach($result->result_array() as $row)
			{
				...
				*/
				...
				//echo "sds";exit;
					...
					$content .= '<a class="menu-instruction" id="image_'.$i.'" href="'.base_url().'index.php/media/index/'.$i.'" ><span class="menu'.$i.'" id="menu_id_'.$i.'"><p>Right Click for More...</p><img  style="width: 200px; height: 150px;" src="'.$image_path.$row["image"].'" id="temp_image_'.$i.'" name="para_'.$i.'" onclick="box_load('.$box_load_parm.')" title = " " alt = ""  /></span></a><input type="hidden" name="control_type[]" value="image" /><input type="hidden" id="input_temp_image_'.$i.'" name="content[]" value="'.$row["image"].'" /> <input type="hidden" name="image-para[]" value="" /><div class="contextMenu" id="myMenu'.$i.'" style="display:none;"><ul><li id="properties"><a id="custom_open" href="'.base_url().'index.php/media/load_properties/'.$i.'"> </a> <img src="'.base_url().'images/properties.png"/> Image Properties</li></ul></div>';
				...
				if($row['type']=="image-para")
				{
					...
					$content .= '<div class="contextMenu" id="myMenu'.$i.'" style="display:none;"><ul><li id="properties"><a id="custom_open" href="'.base_url().'index.php/media/load_properties/'.$i.'"> </a> <img src="'.base_url().'images/properties.png"/> Image Properties</li></ul></div>';
					$content .= '<input type="hidden" name="temp_data'.$i.'" id="temp_data'.$i.'" value="'.htmlentities($row['data']).'" disabled="disabled">'  ;
				...
				/*
				Commenting Numaan's Code By Usman Saleem
				...
				if($row['type']=="image")
				{
					...
					$content .= '<div><img src="'.$image_path.$row['image'].'" /></div>';    
				}
				if($row['type']=="image-para")
				{
					$content .= '<div style="clear:both;height:auto;"><p><img style="margin-right:10px;" align="left" src="'.$image_path.$row['image'].'" />'.$row['data'].'</p></div>';
