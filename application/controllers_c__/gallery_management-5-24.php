...
{ 
		...
		for($i=1; $i<= intval($numImages); $i++)
		{
			...
			if($_FILES['slide_image'.$i]['tmp_name']!='')
			{
			   ...
			   $slide_description = $_POST['slide_description'.$i];
				...
				$config['upload_path'] 	= realpath('.').'/media/ckeditor_uploads/'.$_SESSION['user_info']['user_login'].'_'.$_SESSION['user_info']['user_id'].'/galleries';             
			  ...
			  //  $this->image_lib->clear(); 
				...
				for($j=0; $j < count($array_thumb_size); $j++)
				{
					...
					{  				   
						...
						$config_resize['new_image'] = realpath('.').'/media/ckeditor_uploads/'.$_SESSION['user_info']['user_login'].'_'.$_SESSION['user_info']['user_id'].'/galleries';
						if(!is_dir($config['upload_path']))
						  {
							 mkdir($config['upload_path'],0777,true);
						  }
	...
	function delete_slide_image_info($slide_image_id)
	{
		...
		$this->checkLogin();
		$this->Gallery_Model->delete_slide_image_info($slide_image_id);
