...
class Pages_Model extends CI_Model{
	...
	{   
		...
		$this->db->query("INSERT INTO menuitems(item_name, item_published, item_status, item_parent ) VALUES(".$this->db->escape($item_name).", 'Yes', 'Active', ".$this->db->escape($parent_id).")");
		$item_id = $this->db->insert_id();
	...
	function deletePages($arrPageId)
	{
		for( $i=0; $i<sizeof($arrPageId); $i++)
		{
			...
			foreach($array_rslt_item_id as $row_item_id)
			{
				...
				$qry_del_mpx = "DELETE FROM menuitems_pages_xref WHERE item_id=".$this->db->escape($page_id);
	...
	//print_r($_POST);
		...
		for($i=0; $i<$numControls; $i++)
		{
			...
			{       
				...
				{
					$qryPageContent = "INSERT INTO page_content_controls(page_id, data,image, type,image_url,image_target,image_alt,image_width,image_height,image_border,image_hspace,image_vspace,image_alignment) 
										VALUES(
