	  ...
	  <label class="right_bar_label"><?=$right_menus[$i]['menu_name']?></label>
		<br clear="all" /> 
		   <div class="links">
            ...
            if(sizeof($form_links) > 0)
            {	
				?>
      		  <ul>
                <?php
                foreach($form_links as $keys => $values)
                {
                  // echo "<pre>";
				   //print_r($form_links[$keys]['link']);
				  /* echo $keys;
				   echo "<pre>";
				   print_r($values);*/
				   if($mode=='edit')
                   ...
                   {
                       $strLink = base_url()."index.php/".$form_links[$keys]['link'];
                   }
                ?>
                   <li><a href="<?=$strLink?>"><?=$form_links[$keys]['title']?></a></li>
            <?php
