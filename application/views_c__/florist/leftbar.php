...
?>
		   ...
		   <div class="links">
			...
			{?>
				...
				<?php
					...
					// echo "<pre>"; print_r($left_menus); echo "</pre>";
		...
		<?
			...
			if(isset($form_links_left_left) && sizeof($form_links_left) > 0)
            {	
				?>
      		  <ul>
                <?php
				for($x=0; $x<sizeof($form_links_left); $x++)
               	{ 
					foreach($form_links_left[$x] as $keys => $values)
					{
					   if($mode=='edit')
					   {
						   $strLink = 'javascript: void(0);';    
					   ...
					   {
						   $strLink = base_url().$form_links_left[$x][$keys]['link'];
					   }
					?>
					   <li><a href="<?=$strLink?>"><?=$form_links_left[$x][$keys]['title']?></a></li>
				<?php
					}				   
                }?>
        	</ul>    
