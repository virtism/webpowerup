	...
	<li>
		<h3>
         <?php if($this->config->item('seo_url') == 'On'){?>
                <a style="text-decoration: none;" href="http://<?=$_SERVER['SERVER_NAME'];?>/MyShop/detail/<?=$p['product_id']?>" ><?php echo $p['product']; ?></a></h3> 
         <?php }
         else{?>
                <a style="text-decoration: none;" href="<?=base_url().index_page()?>MyShop/detail/<?=$p['product_id']?>" ><?php echo $p['product']; ?></a></h3> 
         <?php }?>
        <!--<a style="text-decoration: none;" href="<?=base_url().index_page()?>MyShop/detail/<?=$p['product_id']?>" ><?php echo $p['product']; ?></a></h3>-->
		<?php if ($p['image']){
                  if($this->config->item('seo_url') == 'On'){?>
		                <a href="http://<?=$_SERVER['SERVER_NAME'];?>MyShop/detail/<?=$p['product_id']?>" ><img src="<?php echo base_url(); ?>media/ckeditor_uploads/<?=$_SESSION['user_info']['user_login'].'_'.$_SESSION['user_info']['user_id']?>/ecommerce/<?=$img_path_thumb?>/<?php echo $p['image']; ?>" border="0" alt="" width="116px" height="125px" /> </a> 
	             <?php }
                 else{?>
                ...
                <?php if($this->config->item('seo_url') == 'On'){?>
                        <a href="http://<?=$_SERVER['SERVER_NAME'];?>/MyShop/detail/<?=$p['product_id']?>" ><img src="<?php echo base_url(); ?>media/ecommerce/no-image.png" border="0" alt="" width="116px" height="125px" /> </a> 
                 <?php }
                 else{?>
		...
		<small>$<?php echo $p['list_price']; ?></small>
			<fieldset>
				...
				<?php 
                    ...
                    if($this->config->item('seo_url') == 'On')
                     {
                        echo form_open('http://'.$_SERVER['SERVER_NAME'].'/MyShop/add_cart_item',$attributes);
                     }
                     else
                     {
				...
				<?php
                    if($this->config->item('seo_url') == 'On')
                    {
                        echo anchor('http://'.$_SERVER['SERVER_NAME'].'/MyShop/detail/'.$p['product_id'], 'Detail View');
                    }
                    else
                    {
                        echo anchor(base_url().index_page().'MyShop/detail/'.$p['product_id'], 'Detail View');    
                    }
                ?>  
		   <?php echo form_close(); 
		   ?>  
...
<div style="float:left;  width:160px; height: auto;"><!--ForEach Start Div -->	
	<div style="font-size:14px; width:auto; text-align:center; height: auto;"> 
    <h3>
        <?php if($this->config->item('seo_url') == 'On'){?>
                <a style="text-decoration: none;" href="http://<?=$_SERVER['SERVER_NAME'];?>/MyShop/detail/<?=$p['product_id']?>" ><?php echo $p['product']; ?></a></h3> 
         <?php }
