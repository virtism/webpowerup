...
?>      <?=$editIcon?>
		<h1><?=$left_menus[$i]['menu_name']?></h1>
			...
			?>
				...
				if(isset($left_sub_menus[$i]) && sizeof($left_sub_menus[$i]) > 0)
				{/*		 
               		?>
                <!--4 April - Child Menu-->
                <h2><?=$left_sub_menus[$i]['menu_name']?></h2>
                <li id="menu_area_<?=$i?>">
                	<ul>
					<?
					if(sizeof($left_sub_menus) > 0)
					{
					  	//echo "i am here";
						for($j=0; $j<sizeof($left_sub_menus[$i]['sub_menu']); $j++)
                        {
							//echo "<pre>";
							...
							{
								$strLink = base_url()."index.php/site_preview/page/".$site_id."/".$left_sub_menus[$i]['sub_menu'][$j]['page_id'];
							}
							
							if(empty($left_sub_menus[$i]['sub_menu'][$j]["page_id"]) && !empty($left_sub_menus[$i]['sub_menu'][$j]["item_link"]))
							{
								//Checking The Full URL 
								if(strstr($left_sub_menus[$i]['sub_menu'][$j]["item_link"],"http://"))
								{
									$link_url =  $left_sub_menus[$i]['sub_menu'][$j]["item_link"];	
								}
								else
								{
									//Making Full Url Starting from HTTP
									$link_url =  "http://".$left_sub_menus[$i]['sub_menu'][$j]["item_link"];	
								}
								//exit;
							}
							else
							{
                        	    ...
                        	    ?>
                   	        <li><a href="<?=$link_url?>"><?=$left_sub_menus[$i]['sub_menu'][$j]['item_name']?></a></li>
					<?php }
					} ?>
					</ul>
				</li> 
