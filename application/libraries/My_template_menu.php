...
//       echo "<pre>";print_r($arrMenus1[$i]);exit; 
	...
	//	exit;
		...
		if(isset($_SESSION['login_info']["customer_id"]))
		{
			...
			$arrMenus5 = $resultMenu->result_array();
			// echo "here";
			// echo "<pre>";	print_r($arrMenus5);	echo "</pre>";
			for($i=0; $i<sizeof($arrMenus5); $i++)
			{
		...
		//For Getting Webinar Lists
		if(count($finalArray) > 0)
		{
			for($i=0; $i<count($finalArray) ;$i++)
			{
				//echo $finalArray[$i]["menu_id"]."<br>";
				$menu_id = $finalArray[$i]["menu_id"];
				
				$menus_webinars = $obj->Menus_Model->get_top_webinar_items($menu_id);
				if(count($menus_webinars) > 0)
				{
					$finalArray[$i]["webinar_list"] = $menus_webinars;
				}	
			}
		}	
	   ...
	   // $data['color_scheme'] = $this->Menus_Model->get_scheme_color($site_id);
		...
		if(array_key_exists('default',$color_scheme) && $color_scheme['default'] != '' && $color_scheme['default'] != 'Default'){
		 ...
		 //	';
		  ...
		  /*	!important attribute was added to over come default color problem 		*/
		  if($temp_name == "mashup")
		  {
				$my_css_anchor = "
				.menu4 ul > li{
					background-color: #".$anchor_backgroud." !important;
					margin:2px 5px 0 0; 
				}
				.menu4 ul > li:hover{
					background-color: #".$anchor_back_active." !important;
					margin:2px 5px 0 0; 
				}
				.menu4 ul li ul li{
					color: #".$anchor_txt." !important;
					background-color: #".$anchor_backgroud." !important;
					margin:0px 0px 0 0; 
				}
				.menu4 ul li ul li:hover{
					color: #".$anchor_back_txt." !important;
					background-color: #".$anchor_back_active." !important;
					margin:0px 0px 0 0; 
				}
				.menu4 a{
		  ...
		  else
		  {
			...
			{
				color: #'.trim($anchor_txt).' !important;
				background-color: #'.trim($anchor_backgroud).' !important;
			}
			/* a.link_style:visited {color: #'.trim($anchor_txt).' !important;  background-color: #'.trim($anchor_backgroud).' !important;} */
			a.link_style:hover 
			{
				color: #'.trim($anchor_back_txt).' !important;
				background-color: #'.trim($anchor_back_active).' !important;
			}
			/* a.link_style:focus {color: #'.trim($anchor_txt).' !important;  background-color: #'.trim($anchor_backgroud).' !important;}  */
			a.link_style:active , a.link_style:hover 
			{
				color: #'.trim($anchor_back_txt).' !important;
				background-color: #'.trim($anchor_back_active).' !important;
			}
...
function getSiteTemplate_id($site_id)
	{
		 ...
		 $obj =& get_instance(); 
		 return $id = $obj->Site_Model->getSiteTemplate_id($site_id);
		 
		 // $id = $this->Site_Model->getSiteTemplate_id($site_id);
	}
