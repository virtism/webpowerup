<?
//echo "<pre>";
//echo $form_links_menu_id;
//print_r($form_links);
//print_r($left_menus[0]['menu_id']);
//echo sizeof($left_menus);
//exit;
?>
<style>
.box-head
...
?>
			...
			<div class="side_bar">
		  <label class="side_bar_label"><?=$left_menus[$i]['menu_name']?></label>
			<br clear="all" /> 
		   <div class="links">
			<?php
		...
		<?
			...
			if(isset($form_links) && sizeof($form_links) > 0)
            {	
				?>
      		  <ul>
                <?php
				for($i=0; $i<sizeof($form_links); $i++)
				{
					foreach($form_links[$i] as $keys => $values)
					{
					   if($mode=='edit')
					   {
						   $strLink = 'javascript: void(0);';    
					   ...
					   {
						   $strLink = base_url()."index.php/".$form_links[$i][$keys]['link'];
					   }
					?>
					   <li><a href="<?=$strLink?>"><?=$form_links[$i][$keys]['title']?></a></li>
						<?php
					}	   
				 }	?>
...
if(isset($left_menus_type)){
	...
	echo '<div class="side_bar">';	
	 echo '<label class="side_bar_label">Categories</label>';
	  echo '<br clear="all" />';
