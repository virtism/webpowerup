...
class Webinar_Model extends CI_Model{
	...
	function creat_new_webinar($site_id)
	{
		...
		$this->send_mail_attendee($webinar_id, $_REQUEST["title"], $_REQUEST["ck_content"],$_REQUEST["start_time"], $_REQUEST["webinar_date"], $groupids,  $_REQUEST["webinar_access"], $site_id, $new_customers_id, $webinar_rid);
...
OR ec_customers_group_xref.group_id =".intval($id)."");
	 ...
	 function get_all_customer_by_site_id($id, $all_user=0){
	  ...
	  {
	  	 $Q = $this->db->query('SELECT * FROM  `ec_customers` WHERE  `customer_id` ='.$id. ' AND  `site_id` ='.$all_user);	  exit;
	  }
	  else
	  {
	...
	//Send email function
	function send_mail_attendee($webinar_id, $title, $content, $start_time, $start_date, $group_ids='', $webinar_access, $site_id, $new_customers_id, $webinar_rid)
		{
			...
			$all_customer = array();
				...
				$body = $message."<b>Presenter Meeting Email</b><br/><a href='http://webpowerup.com/broadcast/GWSWhiteboard.html#roomID=".$webinar_rid."&attendee_id=".($_SESSION['user_info']['user_id']*2)."'>Join Webinar</a>";
			...
			{		
						...
						foreach($group_ids as $gid)
						{
							...
							{	
								...
								$body = $dear.$message."<br/><a href='http://webpowerup.com/broadcast/GWSWhiteboard.html#webinarID=".$webinar_rid."&attendee_id=".$customer_rec[$i]['customer_id'] * 2 ."'>Join Webinar</a>";
			...
			else if(isset($webinar_access) && $webinar_access == 'Everyone')
			{
				...
				for($i = 0; $i<count($customer_rec); $i++)
				{
					...
					$body = $dear.$message."<br/><a href='http://webpowerup.com/broadcast/GWSWhiteboard.html#webinarID=".$webinar_id."&attendee_id=".$customer_rec[$i]['customer_id']* 2 ."'>Join Webinar</a>";
					$all_customer[] = $customer_rec[$i]['customer_id'];
					$this->email->from('join_meeting@webpowerup.com', 'WebpowerUp');
					$this->email->subject($subject);
					$this->email->message($body);
			...
			/* user id append  3-13-2012 */
			if(isset($_SESSION['user_info']['user_id']))
			{
				$save_ids = $save_ids.",".$_SESSION['user_info']['user_id'];
			}
			/* user id append  3-13-2012 */
  ...
  //===== by : mudassar ali      ===== //
   ...
   function add_customer_by_webinar($site_id='')
	...
	function check_customer_webinar($site_id, $attendee_id)
	{
		...
		$exist = 0;
		//echo $customer_id;
		$query_string = "SELECT id,attendee_id FROM webinar where site_id = ".$site_id;
	...
	function get_user_webinar($site_id, $attendee_id)
	{
		...
		$query_string = "SELECT * FROM webinar where site_id = ".$site_id;
        $q = $this->db->query($query_string);
        if($q->num_rows() > 0)
		{
			$webinar_data = $q->result_array();
