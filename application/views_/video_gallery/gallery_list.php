...
{    
    if(isChecked() == true)
    {
        ...
        form.action = "<?=base_url().index_page()?>video_gallery/publishGallery/<?=$site_id?>";
        form.submit();
    }
    else
    {
...
{    
    ...
    {
        form = document.getElementById('frmSlides');
        form.action = "<?=base_url().index_page()?>video_gallery/unpublishGallery/<?=$site_id?>";
        form.submit();
    }
    else
    {
...
{    
    if(isChecked() == true)
    {
		  ...
		  {
		    form = document.getElementById('frmSlides');
            form.action = "<?=base_url().index_page()?>video_gallery/trashGallery/<?=$site_id?>";
            form.submit();
			return true;
		  }	 
          else
...
<div class="RightColumnHeading">
    <h1>
     ...
     <div class="RightSideButton">
        ...
        <a href="<?=base_url().index_page();?>video_gallery/create_video_gallery/<?=$site_id?>">
...
<div class="DataGrid2">
		...
		if($records)
		{
			...
			<ul>
				...
				<li>
                    <a href="<?=base_url().index_page();?>video_gallery/edit/<?=$site_id?>/<?=$row['gallery_id']?>" class="activelink">
                    <?=$row['gallery_title'];?>
                	</a>
				</li>
				<li>
