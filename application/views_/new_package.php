	 ...
	 <td>
		...
		<?
			...
			for($i = 0; $i<count($modules); $i++ )
			{
					...
					/*echo $moduleArray[$i]["module_id"]."-----".$modules[$i]["module_id"]."<br>";
					
					if(array_search($moduleArray[$i]["module_id"],$modules[$i]) )
					{
					...
					{                              
						...
						if(isset($moduleArray) && count($moduleArray > 0))
						{
							for($j=0;$j<count($moduleArray);$j++)
							{
									
								//echo "<br>".$modules[$i]["module_id"]."-------".$moduleArray[$j]["module_id"]."<br>";     
								
							//	if(array_search($modules[$i]["module_id"],$moduleArray[$j]) )
							if($modules[$i]["module_id"] == $moduleArray[$j]["module_id"])
								{
									$checked = 'checked= "checked"';  
								}
								else
