<? if(isset($cat_title))
	{  
		echo '<h1 style="font-size:16px; padding: 2px 0 0 15px;">'.$cat_title.'</h1>'; 
	}
?>
...
if (count($products) < 1){
	if($ishome == 'yes')
 	...
 	{
		echo '<p style="padding: 0 0 0 15px;">No Product in this category !</p>'; 
 	}  
					}
?>
		...
		<small>&euro;<?php echo $p['list_price']; ?></small>
			<fieldset>
				...
				<?php 
					...
					{
						echo "<a target='_blank' href=".$p['products_attributes_url']." >Download Here</a>";
					}
					else if(isset($p['products_attributes_filename']) && !empty($p['products_attributes_filename']))
					{
						
						echo "<a target='_blank' href=".base_url()."media/ecommerce/downloadproducts/".$p['products_attributes_filename'].">Download Here</a>";
					}
					else
					{
...
<div style="float:left;  width:160px; height: auto;">
    <div style="font-size:14px; width:auto; text-align:center; height: auto;"> <h3><a style="text-decoration: none;" href="<?=base_url().index_page()?>MyShop/detail/<?=$p['product_id']?>" ><?php echo $p['product']; ?></a></h3>
	</div>
